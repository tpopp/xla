  ...
  std::function<void(HloComputation*,
      ...
      process_computation = [this, &process_computation, pressure_trackers](
        ...
        for (int idx = (top_down_scheduling_ ? 0 : sequence.size() - 1);
          ...
          tracker->PrecomputeCalledComputationsPeak(instruction);
...
void MemoryPressureTracker::PrecomputeCalledComputationsPeak(
    const HloInstruction* instruction) {
  auto id = GetInstructionId(instruction);
  auto& s = alloc_release_spans_[id];
  s.called_computations_peak = ComputeCalledComputationsPeak(instruction);
}
...
void MemoryPressureTracker::Initialize(
  ...
  for (auto* instruction : computation->instructions()) {
    ...
    // Initialize called_computations_peak to -1 to indicate that it has not
    // been computed yet.
    s.called_computations_peak = -1;
...
void MemoryPressureTracker::UpdateBuffers(const HloInstruction* instruction) {
  auto instruction_id = GetInstructionId(instruction);
  int64_t computations_peak =
      alloc_release_spans_[instruction_id].called_computations_peak;
  DCHECK_NE(computations_peak, -1);
...
int64_t MemoryPressureTracker::ComputeCalledComputationsPeak(
    const HloInstruction* instruction) {
  int64_t peak = 0;
  ...
  if (!instruction->called_computations().empty()) {
    for (auto* called_comp : instruction->called_computations()) {
      ...
      peak = std::max(peak, it->second.memory_peak);
    }
  }
  return peak;
}
...
std::pair<int64_t, int64_t> MemoryPressureTracker::MemoryPressureDifference(
  ...
  // Compute peak increase produced by called computations.
  auto instruction_id = GetInstructionId(instruction);
  int64_t peak = alloc_release_spans_[instruction_id].called_computations_peak;
  DCHECK_NE(peak, -1);
