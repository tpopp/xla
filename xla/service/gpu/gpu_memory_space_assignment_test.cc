...
TEST_P(GpuMosaicCollectiveMemorySpaceAssignmentTest,
  ...
  const absl::string_view kMosaicMetadataModule = R"(
    HloModule m
    ENTRY main {
      ROOT %custom-call.9 = (f16[8], f16[8]) custom-call(), custom_call_target="mosaic_gpu_v2", backend_config={uses_xla_collective_metadata=true}
    }
  )";
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      ...
      ParseAndReturnVerifiedModule(IsMosaicWithCollectiveMetadata()
                                       ? kMosaicMetadataModule
                                       : kMosaicModule,
                                   config));
  AliasInfo alias_info;
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloAliasAnalysis> alias_analysis,
